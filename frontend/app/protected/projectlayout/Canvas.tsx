...
const buttonMap: { [key: string]: any[] } = {
    CRUD: [
        new Blocks.SELECT(5, 50),
        new Blocks.SELECT_DISTINCT(5, 110),
        new Blocks.UPDATE(5, 170),
        new Blocks.DELETE(5, 230),
        new Blocks.DROP_TABLE(5, 290),
        new Blocks.DROP_COLUMN(5, 350),
    ],
    ...
    Conditions: [
        new Blocks.Equals(5, 50),
        new Blocks.LessThan(5, 110),
        new Blocks.LessEqualTo(5, 170),
        new Blocks.GreaterThan(5, 230),
        new Blocks.GreaterEqualTo(5, 290),
        new Blocks.AND(5, 350),
        new Blocks.OR(5, 410),
        new Blocks.LIKE(5, 470),
    ],
    Operators: [
    ...
    Joins: [new Blocks.Inner_Join(5, 50), new Blocks.Left_Join(5, 110), new Blocks.Right_Join(5, 170), new Blocks.Full_Outer_Join(5, 230)],
};
...
const buttons = [
    new Blocks.BUTTON(5, 5, "#94a3b8", "CRUD"), // UI slate-400 equivalent
    new Blocks.BUTTON(105, 5, "#84cc16", "Identifier"), // UI sage-400 equivalent
    new Blocks.BUTTON(245, 5, "#38bdf8", "Conditions"), // UI sky-400 equivalent
    new Blocks.BUTTON(395, 5, "#ef4444", "Operators"), // UI terracotta-400 equivalent
    new Blocks.BUTTON(535, 5, "#a78bfa", "Joins"), // UI lavender-400 equivalent
];
...
const compoundItems = [
    ...
    "DIV",
    "SUB IDENTIFIER EXPR",
    "MULT IDENTIFIER EXPR",
    "DIVIDE IDENTIFIER EXPR",
    "INNER JOIN",
    "LEFT JOIN",
    "RIGHT JOIN",
    "FULL OUTER JOIN",
...
function displayBlock(ctx: any, block: any) {
    ...
    if (
        block.type === "RawText" ||
        block.type === "BUTTON" ||
        block.type === "IdentifierBox" ||
        block.type === "ColumnReferenceBox" ||
        block.type === "ExpressionBox" ||
        block.type === "TableBox" ||
        block.type === "ConditionBox" ||
        block.type === "WILDCARD" ||
        block.type === "Table" ||
        block.type === "IdentifierInput" ||
        block.type === "ColumnReference" ||
        block.type === "SELECTBox"
    ) {
        ...
        if (block.type !== "RawText") {
            ctx.lineWidth = 2;
            ctx.strokeStyle = "#1e293b"; // UI navy-800 equivalent
            ctx.stroke();
        }
        ctx.font = block.type === "BUTTON" ? "20px Arial" : "18px Arial";
        ctx.fillStyle = block.type === "BUTTON" ? "#0f172a" : block.type === "RawText" ? "#0f172a" : "#ffffff"; // Navy-900 for buttons/text, white for others
        // Center text
...
export default function Canvas({ shapes = [], width = 0, height = 0, blocksRef, toSQLRef }: CanvasProps) {
    ...
    useEffect(() => {
        for (let b = 0; b < blocks.length; b++) blockUpdate(blocks[b]);
        // grab a context
        displayTag();
    }, []);
    ...
    const updateAll = () => {
        ...
        displayTag();
        // draw a giant line
        ctx.fillStyle = "hsl(var(--ui-navy-200))";
        ...
        ctx.strokeStyle = "hsl(var(--ui-terracotta-800))";
        ctx.lineWidth = 2;
    ...
    const getBlockAt = (x: number, y: number) => {
        return blocks.findIndex((block) => x >= block.x && x <= block.x + block.w && y >= block.y && y <= block.y + block.h);
    };
    const getButtonsAt = (x: number, y: number) => {
        return buttons.findIndex((block) => x >= block.x && x <= block.x + block.w && y >= block.y && y <= block.y + block.h);
    };
    ...
    const isBlockInDeleteBox = (block: any): boolean => {
        ...
        return (
            blockCenterX >= deleteBox.x && blockCenterX <= deleteBox.x + deleteBox.w && blockCenterY >= deleteBox.y && blockCenterY <= deleteBox.y + deleteBox.h
        );
    };
    ...
    const handleMouseDown = (e: React.MouseEvent) => {
        ...
        if (button !== -1) {
            ...
            for (let i = 0; i < toolbox.length; i++) {
                ...
                if (x >= toolboxItem.x && x <= toolboxItem.x + toolboxItem.w && y >= toolboxItem.y && y <= toolboxItem.y + toolboxItem.h) {
    ...
    const handleMouseMove = (e: React.MouseEvent) => {
        ...
        setBlocks((prev) => prev.map((block, i) => (i === draggedIndex ? { ...block, x: newX, y: newY } : block)));
        ...
        if (draggedBlock) {
            ...
            if (isBlockInDeleteBox(tempBlock)) {
                deleteBox.color = "hsl(var(--ui-terracotta-600))"; // Brighter terracotta when hovering
            } else {
                deleteBox.color = "hsl(var(--ui-terracotta-700))"; // Normal terracotta
            }
        }
    };
    ...
    return (
        <canvas
            ...
            height={height}
            style={{
                border: "1px solid hsl(var(--ui-navy-300))",
                background: "hsl(var(--ui-beige-200))",
                borderRadius: "0.5rem",
